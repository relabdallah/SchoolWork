...
{ 
 ...
 public: // prototypes to be used by the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: provide the top variable to receive the top element (pass by ref)
  void topElem(el_t&);
  
  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: Checks to see if the stack is empty, returns true if empty
  bool isEmpty();
  //PURPOSE: Checks to see if the stack is full, returns true if full
  bool isFull();
  //PURPOSE: Displays they entire stack
  void displayAll();
  //PURPOSE: Clears and empties the entire stack
  void clearIt();
